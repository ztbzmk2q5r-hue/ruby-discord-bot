...
def build_messages(display_name: str, history: list, user_text: str, chichi: bool, homecoming: bool):
    ...
    # ★帰宅挨拶のゲート（ここがループ停止の本体）
    if homecoming:
        msgs.append({
            "role": "system",
            "content": "今回の発言は帰宅の挨拶。返答で「おかえり」を言ってよいが、1回だけ。以降の返信で繰り返さない。"
        })
    else:
        msgs.append({
            "role": "system",
            "content": "今回の発言は帰宅の挨拶ではない。「おかえり」「ただいま」など帰宅系の挨拶は禁止。"
        })
...
async def on_message(message: discord.Message):
    ...
    # ★重複防止：直近が今の発言なら履歴から外す（同一発言2回問題対策）
    ...
    messages = build_messages(display_name, history, text, chichi, homecoming)
